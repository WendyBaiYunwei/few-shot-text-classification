...
from ordered_data_loader import OrderedTrainDataLoader
        ...
        for type in ['pos', 'neg']:
            ...
            for i in range(len(one_type_file)):
                ...
                for j in range(len(one_type_file)):
                    ...
                    processed2 = processed_data[filename][type]['data'][j]
                    sim_and_value = [sim]
                    sim_and_value.extend(processed2)
                    adj[key].append(sim_and_value)
                adj[key].sort(key=lambda x:x[0], reverse = True) #higher similarity in front
            one_type[type] = adj
        sorted_adjs[filename] = one_type
    return sorted_adjs

def get_train_loader(train_data, support, query, pad_idx, train_domains):
    sorted_adj = get_sorted_adj(train_domains, train_data)
    ...
    return OrderedTrainDataLoader(train_loaders, sorted_adj, support=support, query=query, pad_idx=pad_idx)
...
def main():
    ...
    train_loader = get_train_loader(train_data, support, query, pad_idx, train_domains)
